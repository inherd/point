// based on https://github.com/Peltoche/vixi with Apache
//                                  Apache License
...
impl ClientToClientWriter {
    pub fn send_rpc_notification(&mut self, method: &str, params: &Value) {
        let raw_content = match serde_json::to_vec(&json!({"method": method, "params": params})) {
            Ok(raw) => raw,
            Err(err) => {
                log::error!("failed to create the notification {}: {}", method, err);
                return;
            }
        };
        ...
        match self.0.write(&raw_content) {
            Ok(_) => (),
            Err(err) => log::error!("failed to send the notification {}: {}", method, err),
        };
    }
}
